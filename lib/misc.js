...
exports.randstr = function(len) {
  ...
  bytes = crypto.pseudoRandomBytes(len);
...
/**
 * Returns target object extended with all attributes in the one or more
 * If the last argument is an Array, it will act as a list of keys that
 * are whitelisted to be copied. It is important to note that the order
 * that objects are passed into function is important in regards to how
 * attribute collisions are dealt with.
 * @param {Object} target Base object to be extended
...
exports.extend = function(target) {
  ...
  if (!args.length) {
    throw new Error('Incorrent use of the API, use at least two operands');
  }
  ...
  for (ii = 0; ii < args.length; ii++) {
    ...
    for (key in obj) {
      if (obj.hasOwnProperty(key)) {
        if (!whitelist.length || whitelist.indexOf(key) !== -1) {
          target[key] = obj[key];
        }
      }
    }
